...
const API_URL_BASE = "https://opentdb.com/api.php?";
...
async function fetchFootballQuestions() {
  ...
  const category = 21; 
  const type = "multiple"; 
  const encode = "url3986"; 
  const lang = "es"; //

  
  const url = `${API_URL_BASE}amount=${amount}&category=${category}&type=${type}&encode=${encode}&lang=${lang}`;
...
function drawChart() {
  ...
  const names = lastFiveResults.map((r) => r.name); 
  const scores = lastFiveResults.map((r) => r.score); 
...
// Llama a esta función al cargar la página para mostrar los resultados anteriores
showLastResults();